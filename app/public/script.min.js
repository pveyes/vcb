...
return console.log("createSimpleBlock Finish"),blobSimpleBlock},addFrame=function(frame,type,duration){var data=new Object;switch(data.timecode=duration,data.frame=frame,type){case"video":data.trackid=1,data.flags=128;break;case"audio":data.trackid=2,data.flags=0}queryFrame.push(data),2==fileWriter.readyState&&processWriteFileFromQuery()};fileWriter.onwrite=function(){processWriteFileFromQuery()};return{init:initNginX,add:addFrame,stop:stopRecord,update:updateEBML}},recordWebM=function(stream){var statusRecording,fileWriter,reqID,timeCurrent,acsp,inputStream=stream,recordPixelHeight=240,recordPixelWidth=320,canvas=document.createElement("canvas");canvas.width=recordPixelWidth,canvas.height=recordPixelHeight;var context=canvas.getContext("2d"),videoLocalStream=document.createElement("video");videoLocalStream.src=window.URL.createObjectURL(inputStream),videoLocalStream.autoplay=!0,videoLocalStream.muted=!0,videoLocalStream.height=recordPixelHeight,videoLocalStream.width=recordPixelWidth;var parseWebP=function(riff){for(var VP8=riff.RIFF[0].WEBP[0],frame_start=VP8.indexOf("*"),i=0,c=[];4>i;i++)c[i]=VP8.charCodeAt(frame_start+3+i);var width,horizontal_scale,height,vertical_scale,tmp;return tmp=c[1]<<8|c[0],width=16383&tmp,horizontal_scale=tmp>>14,tmp=c[3]<<8|c[2],height=16383&tmp,vertical_scale=tmp>>14,{width:width,height:height,data:VP8,riff:riff}},parseRIFF=function(string){for(var offset=0,chunks={};offset<string.length;){var id=string.substr(offset,4),len=parseInt(string.substr(offset+4,4).split("").map(function(i){var unpadded=i.charCodeAt(0).toString(2);return new Array(8-unpadded.length+1).join("0")+unpadded}).join(""),2),data=string.substr(offset+4+4,len);offset+=8+len,chunks[id]=chunks[id]||[],chunks[id].push("RIFF"==id||"LIST"==id?parseRIFF(data):data)}return chunks},strToBuffer=function(str){return new Uint8Array(str.split("").map(function(e){return e.charCodeAt(0)}))},reqAnime=function(time){context.drawImage(videoLocalStream,0,0,recordPixelWidth,recordPixelHeight);var delta=time-timeCurrent;timeLast=timeCurrent,timeCurrent=time;var dataImage=canvas.toDataURL("image/webp",1),dataBinaryImageWebP=parseWebP(parseRIFF(atob(dataImage.slice(23)))),duration=Math.round(delta);if(duration>0){var dataImageWebp=strToBuffer(dataBinaryImageWebP.data.slice(4));WEBM.add(dataImageWebp,"video",Math.round(delta))}reqID=requestAnimationFrame(reqAnime)},initRecord=function(){var handlingCreateWriter=function(){console.log("Error: Create Writter")};chrome.fileSystem.chooseEntry({type:"saveFile",suggestedName:"3.webm"},function(writableFileEntry){writableFileEntry.createWriter(function(writer){fileWriter=writer,statusRecording=0,startRecord()},handlingCreateWriter)})},startRecord=function(){switch(console.log(statusRecording),statusRecording){case 0:statusRecording=1;break;case 1:return console.log("Error: It has been recording. Cant start recording in parallel.");case 2:return console.log("Error: Cant start recording, please call init() again");default:return console.log("Error: Cant start recording, please call init() once")}configWebM={PixelHeight:recordPixelHeight,PixelWidth:recordPixelWidth},WEBM=new Ngisx(fileWriter,configWebM),WEBM.init();var ac=new webkitAudioContext,acmss=ac.createMediaStreamSource(inputStream);acsp=ac.createScriptProcessor(16384,2,2),acmss.connect(acsp),acsp.connect(ac.destination),acsp.onaudioprocess=function(e){var audio32Float=e.inputBuffer.getChannelData(0),ab=new ArrayBuffer(2*audio32Float.length),dv=new DataView(ab);for(var i in audio32Float)dv.setInt16(2*Number(i),32767*audio32Float[i]);WEBM.add(ab,"audio",0)},performanceID=performance.now(),timeCurrent=performanceID,reqID=requestAnimationFrame(reqAnime)},stopRecord=function(){acsp.onaudioprocess=function(e){tmp1=e},cancelAnimationFrame(reqID),WEBM.stop(),statusRecording=2};return{init:initRecord,start:startRecord,stop:stopRecord}},dataChannel=function(){var dataChannel={};return dataChannel.msgConstructor=function(data){var tmp=data||new Object;return{t:tmp.t||null,l:tmp.l||null,p:tmp.p||null,s:tmp.s||null,d:tmp.d||null}},dataChannel.receiveMessage=function(d,e){var cekMessage,msgObj=JSON.parse(e.data);if(cekMessage=msgObj?msgObj.t&&msgObj.l&&msgObj.p&&msgObj.s&&msgObj.d:!1){var msgBuffer=[];switch(msgObj.s){case 1:msgBuffer[d.data.from]=msgObj.d;break;case 2:msgBuffer[d.data.from]+=msgObj.d;break;case 3:msgBuffer[d.data.from]+=msgObj.d,dataChannel.joinMessage(msgBuffer[d.data.from],msgObj);break;case 4:msgBuffer[d.data.from]=msgObj.d,dataChannel.joinMessage(msgBuffer[d.data.from],msgObj);break;default:console.log("Warning: No event handler for this message status: "+msgObj.s)}}else console.log("Error: Wrong format data message received")},dataChannel.joinMessage=function(joinMessage,objectMessage){switch(objectMessage.t){case"c":var sender=d.data.client.name,message=joinMessage;chat.receive(sender,message);break;case"p":{var message=JSON.parse(joinMessage),index=parseInt(message.p);message.f}presentation.receive(index,slide);break;case"e":break;default:console.log("Warning: No event handling for this undefined message type :"+objectMessage.t)}},dataChannel.sendMessage=function(channel,data){var msgBuffer=65,res=new dataChannel.msgConstructor(data);res.l=Math.ceil(data.d.length/msgBuffer);for(var i=1;i<=res.l;i++){var positionEnd=i*msgBuffer;if(res.p=i,1==res.l)res.s=4;else switch(i){case 1:res.s=1;break;case res.l:res.s=3;break;default:res.s=2}res.d=data.d.slice(positionEnd-msgBuffer,positionEnd),channel.send(JSON.stringify(res))}},dataChannel}(),chat=function($,dataChannel){var chat={};return chat.receive=function(sender,message){var template=$("#chat-template").html();template=template.replace("{{name}}",sender),template=template.replace("{{message}}",message),$("#chat-list").prepend(template)},chat.send=function(client,text){var message=new dataChannel.msgConstructor(text);message.t="c",message.d=text,console.log("sending message: ",message),dataChannel.sendMessage(client,message)},chat}(jQuery,dataChannel);navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia;var VCB={uid:guid(),version:"3.1",status:null,roomid:null,localStream:null,creatorStream:null,recorder:null,client:null},checkDevice;chrome.storage.local.get(function(item){console.log("Receiving value from local storage..."),item&&item.user?init(item):auth()});var init=function(data){console.log("Local storage data received, checking client data and STUN server"),initClient(data.user),initSTUN(data.stun)},auth=function(){console.log("Local storage data is not received, displaying auth page");var loginPage=$("#login-template").html();$("#main-content").html(loginPage),initAuthListener()},initAuthListener=function(){$("#main-content").off(),$("#main-content").on("submit","#login",function(e){e.preventDefault(),$("#login-button").attr("disabled","disabled").val("LOGGING IN..");var user=$("#user").val();if("undefined"!=typeof user&&user.length>0){var data={user:user};chrome.storage.local.set(data),init(data)}else $("#login-button").removeAttr("disabled").val("LOG IN")})},initClient=function(name){VCB.client={name:name,uid:VCB.uid,version:VCB.version}},socket,initSTUN=function(stun){if("undefined"==typeof stun){var stunSelectPage=$("#setup-template").html().replace("{{user}}",VCB.client.name);$("#main-content").html(stunSelectPage);var defaultTemplate=$("#stun-select").attr("data-default");stunSelection=$("#"+defaultTemplate).html(),$("#stun-select").html(stunSelection),$("#main-content").on("submit","#stun-setup",function(e){e.preventDefault();var stunServer=$("#stun-server").val();console.log("Input STUN server from form: ",stunServer),initSocketConnection(stunServer)}),$("#main-content").on("click",".stun-option",function(e){e.preventDefault();var stunServer=$(this).attr("data-address");if("custom"===stunServer){console.log("selecting custom STUN server, displaying form"),$("#setup").css("height","380px");var customSTUNForm=$("#custom-stun-template").html();$("#stun-select").html(customSTUNForm)}else console.log("selecting STUN server from options: ",stunServer),initSocketConnection(stunServer)}),$("#main-content").on("click","#custom-stun-cancel",function(e){e.preventDefault(),$("#setup").css("height","320px"),$("#stun-select").html($("#stun-select-template").html())})}else console.log("STUN server is already defined: ",stun),initSocketConnection(stun)},resetSTUN=function(){socket&&(socket.disconnect(),socket=void 0,chrome.storage.local.remove("stun")),initSTUN()},registerSocket=function(){socket.on("init-ip",initIP),socket.on("create-room",initStream),socket.on("list-room",showListRoom),socket.on("status-stun",showStatusSTUN)},initDashboard=function(stunServer){clearInterval(checkDevice);var template=$("#dashboard-template").html();template=template.replace("{{user}}",VCB.client.name),template=template.replace("{{display-status}}","on"),template=template.replace("{{camera-status}}","check"),template=template.replace("{{audio-in-status}}","check"),template=template.replace("{{audio-out-status}}","on"),template=template.replace("{{stun-server}}",stunServer),template=template.replace("{{client-ip}}","Checking..."),$("#main-content").addClass("fadeIn").html(template);var contentDefault=$("#"+$("#dashboard-content").attr("data-default")).html();$("#dashboard-content").html(contentDefault),checkDevice=setInterval(function(){checkDeviceStatus()},5e3),$("#main-content").off(),$("#main-content").on("click","#local-content-button",localPlayback),$("#main-content").on("click","#create-room-button",createRoom),$("#main-content").on("click","#join-room-button",joinRoom),$("#main-content").on("click","#select-device-button",selectDevice),$("#main-content").on("click","#leave-room-button",leaveRoom),$("#main-content").on("click","#logout",logout),$("#main-content").one("click",".change-stun-button",function(){$(this).off("click"),resetSTUN()}),$("#main-content").on("mouseover",".main-menu",function(){var hoverClass="hover-"+$(this).attr("data-hover");$("#main-menus").attr("class",hoverClass)})},initIP=function(d){$("#client-ip").html(d.data.IP)},checkDeviceStatus=function(){function gotSources(sources){for(var audio=0,video=0,i=0;i<sources.length;i++){var source=sources[i];"video"===source.kind?video++:"audio"==source.kind&&audio++}$("#audio-in-info").addClass(audio>0?"on":"off"),$("#camera-info").addClass(video>0?"on":"off")}var resetStatus=function(elem){elem.removeClass("on").removeClass("off").removeClass("unknown").addClass("check")};resetStatus($("#camera-info")),resetStatus($("#audio-in-info")),resetStatus($("#stun-info")),$("#stun-info").addClass(socket&&1==socket.socket.connected?"on":"off"),"undefined"==typeof MediaStreamTrack?($("#camera-info").addClass("unknown"),$("#audio-in-info").addClass("unknown")):MediaStreamTrack.getSources(gotSources)},logout=function(e){e.preventDefault(),chrome.storage.local.remove(["user","stun"]);var loginPage=$("#login-template").html();$("#main-content").html(loginPage),initAuthListener()},VCBnet=new Object,Vpeer,pptFiles={data:new Array,size:0},currentSlide=0,localPlayback=function(e){e.preventDefault();var popupForm=$("#local-content-template").html();$("#popup").html(popupForm);var localMediaPlayback=$("#local-playback-template").html();$("#dashboard-content").html(localMediaPlayback),$("#main-content").on("change","#local-content-input",function(){var file=this.files[0],type=file.type,videoNode=document.getElementById("local-playback"),canPlayFile=videoNode.canPlayType(type);if(canPlayFile=""===canPlayFile?"no":canPlayFile,"no"===canPlayFile)console.log("unable to play media");else{var fileURL=window.URL.createObjectURL(file);videoNode.src=fileURL,videoNode.setAttribute("data-playable","playable")}}),$("#main-content").on("submit","#local-content-form",function(e){e.preventDefault(),$("#local-playback-wrapper").show(),$("#popup > .popup").removeClass("fadeIn").addClass("fadeOut").delay(1500).hide(function(){$(this).remove(),$("#local-playback")[0].play()})}),$("#main-content").on("click",".local-playback-close",function(e){e.preventDefault();var defaultDashboardTemplate=$("#dashboard-home-template").html();$("#dashboard-content").html(defaultDashboardTemplate),$("#popup form").removeClass("fadeInDown").addClass("fadeOutUp"),$("#popup > .popup").removeClass("fadeIn").addClass("fadeOut").delay(1500).hide(function(){$(this).remove()})})},selectDevice=function(e){e.preventDefault();var popupForm=$("#select-device-template").html();$("#popup").html(popupForm)},createRoom=function(e){e.preventDefault();var popupForm=$("#create-room-template").html();$("#popup").html(popupForm),$("#main-content").on("submit","#create-room",function(e){e.preventDefault();var roomInfo={name:$("#room-name-input").val(),desc:$("#room-desc-input").val(),creator:VCB.client.name},constraints={audio:!0,video:!0},successCallback=function(stream){VCB.localStream=stream,socket.emit("create-room",VCB.client,roomInfo)},errorCallback=function(error){console.log("initStream error on getUserMedia: ",error)};navigator.getUserMedia(constraints,successCallback,errorCallback),$("#popup > .popup").removeClass("fadeIn").addClass("fadeOut").delay(1500).hide(function(){$(this).remove()})}),$("#main-content").on("change","#room-presentation-input",function(e){pptFiles={data:[],size:0};for(var files=e.target.files,i=0;i<files.length;i++){var reader=new FileReader,file=files[i];reader.onload=function(e){pptFiles.data.push(e.target.result),pptFiles.size+=e.target.result.length},reader.readAsDataURL(file)}}),$("#main-content").on("click","#create-room-cancel",function(e){e.preventDefault(),$("#popup form").removeClass("fadeInDown").addClass("fadeOutUp"),$("#popup > .popup").removeClass("fadeIn").addClass("fadeOut").delay(1500).hide(function(){$(this).remove()})})},showListRoom=function(d){var roomListPage=$("#join-room-template").html();$("#dashboard-content").html(roomListPage);var rooms=d.rooms;for(var i in rooms){var room=$("#list-room-template").html(),roomInfo=rooms[i];room=room.replace("{{room_id}}",roomInfo.id),room=room.replace("{{room_name}}",roomInfo.name),room=room.replace("{{room_desc}}",roomInfo.desc),room=room.replace("{{room_total_connect}}",roomInfo.totalConnect),room=room.replace("{{speaker}}",roomInfo.creator.name),$("#room-list").append(room)}$("#main-content").on("click",".room",function(e){e.preventDefault(),VCB.localStream&&VCB.localStream.stop();var roomID=this.id;VCB.roomid=d.data;for(var roomInfo,i=0;i<rooms.length;i++)if(rooms[i].id==roomID){roomInfo=rooms[i];break}if(!roomInfo)return!1;VCB.roomInfo=roomInfo;var successCallback=function(stream){VCB.localStream=stream,startStream(roomInfo),socket.emit("join-room",roomID)},errorCallback=function(error){console.log("Error getUserMedia:",error)},constraints={audio:!0,video:!0};navigator.getUserMedia(constraints,successCallback,errorCallback)}),$("#main-content").on("click","#join-room-cancel",function(){var defaultDashboardTemplate=$("#dashboard-home-template").html();$("#dashboard-content").html(defaultDashboardTemplate)})},joinRoom=function(e){e.preventDefault(),socket.emit("list-room",VCB.client)},statusSTUN=function(){socket.emit("status-stun")},initMainStream=function(localStreamURL){var mainStream=$("#stream")[0];mainStream.autoplay=!0,mainStream.mute=!0,mainStream.src=localStreamURL},initNewStream=function(remoteSessionID,newStreamURL,muted){var streamRemote=document.createElement("video");streamRemote.id=remoteSessionID,streamRemote.className="stream--remote",streamRemote.autoplay=!0,streamRemote.src=newStreamURL,1==muted&&(streamRemote.mute=!0),$("#peer-stream").append(streamRemote)},startStream=function(roomInfo){var streamT=$("#stream-template").html();streamT=streamT.replace("{{room_name}}",roomInfo.name),streamT=streamT.replace("{{room_desc}}",roomInfo.desc),streamT=streamT.replace("{{room_speaker}}",roomInfo.creator.name),$("#dashboard-content").html(streamT);var localStreamURL=window.URL.createObjectURL(VCB.localStream);VCB.roomInfo=roomInfo,roomInfo.creator.sessionID==socket.socket.sessionid?(initMainStream(localStreamURL),$("#stream").attr("muted","yes"),$("#slide-current").attr("src",pptFiles.data[0]),$("#slide-current-num").html(currentSlide+1+"/"+pptFiles.data.length),currentSlide==pptFiles.data.length-1&&$("#slide-control-next").addClass("disabled"),$("#main-content").on("click",".slide-control",function(e){if(e.preventDefault(),control=$(this).attr("data-control"),"next"==control){if(!(currentSlide<pptFiles.data.length-1))return!1;currentSlide+=1}else{if("prev"!=control)return!1;if(!(currentSlide>0))return!1;currentSlide-=1}currentSlide>0&&$("#slide-control-prev").removeClass("disabled"),0==currentSlide&&$("#slide-control-prev").addClass("disabled"),currentSlide<pptFiles.data.length-1&&$("#slide-control-next").removeClass("disabled"),currentSlide==pptFiles.data.length-1&&$("#slide-control-next").addClass("disabled"),$("#slide-current-num").html(currentSlide+1+"/"+pptFiles.data.length),$("#slide-current").attr("src",pptFiles.data[currentSlide]);for(var client in VCBpeer){var d={data:{to:client,page:currentSlide}};socket.emit("control-presentation",d)}})):initNewStream(socket.sessionID,localStreamURL,!0);var VCBpeerConstructor=function(){return{session:{local:null,remote:null},peer:null,datachannel:null}},msgDataChannelConstructor=function(data){var tmp=data||new Object;return{t:tmp.t||null,l:tmp.l||null,p:tmp.p||null,s:tmp.s||null,d:tmp.d||null}},VCBpeer=new Object,VCBMsgBuff=new Object;Vpeer=VCBpeer;var handleDataChannelReceiveMessages=function(d,e){var cekMessage,msgObj=JSON.parse(e.data);if(cekMessage=msgObj?msgObj.t&&msgObj.l&&msgObj.p&&msgObj.s&&msgObj.d:!1){var handleJoiningReceiveMessages=function(joinMessage,objectMessage){switch(objectMessage.t){case"c":var chat=$("#chat-template").html();chat=chat.replace("{{name}}",d.data.client.name),chat=chat.replace("{{message}}",joinMessage),$("#chat-list").prepend(chat);break;case"p":var message=JSON.parse(joinMessage);pptFiles.data[parseInt(message.p)]=message.f,pptFiles.size+=message.f.length;break;case"e":break;default:console.log("Warning: No event handling for this undefined message type :"+objectMessage.t)}};switch(msgObj.s){case 1:VCBMsgBuff[d.data.from]=msgObj.d;break;case 2:VCBMsgBuff[d.data.from]+=msgObj.d;break;case 3:VCBMsgBuff[d.data.from]+=msgObj.d,handleJoiningReceiveMessages(VCBMsgBuff[d.data.from],msgObj);break;case 4:VCBMsgBuff[d.data.from]=msgObj.d,handleJoiningReceiveMessages(VCBMsgBuff[d.data.from],msgObj);break;default:console.log("Warning: No event handler for this message status: "+msgObj.s)}}else console.log("Error: Wrong format data message received")},sendFilePresentation=function(clientID){var handleSendFileViaSocket=function(clientID,msgFile){var msgBuffer=5e4,res=new msgDataChannelConstructor(data),d={data:{to:clientID,file:null}};res.l=Math.ceil(msgFile.d.length/msgBuffer),res.t="p";for(var i=1;i<=res.l;i++){var positionEnd=i*msgBuffer;if(res.p=i,1==res.l)res.s=4;else switch(i){case 1:res.s=1;break;case res.l:res.s=3;break;default:res.s=2}res.d=msgFile.d.slice(positionEnd-msgBuffer,positionEnd),d.data.file=JSON.stringify(res),socket.emit("init-presentation",d)}};for(var i in pptFiles.data){var msg=new msgDataChannelConstructor;msg.t="p",data={f:pptFiles.data[i],p:i},msg.d=JSON.stringify(data),handleSendFileViaSocket(clientID,msg)}},handleAskOffer=function(d){var joinMessage=$("#join-room-message-template").html().replace("{{name}}",d.data.client.name);$("#chat-list").prepend(joinMessage),VCBpeer[d.data.from]=new VCBpeerConstructor,VCBpeer[d.data.from].peer=new webkitRTCPeerConnection(null,{optional:[{RtpDataChannels:!0}]});var peer=VCBpeer[d.data.from].peer;VCBpeer[d.data.from].datachannel=peer.createDataChannel(d.data.from,{reliable:!0});var datachannel=VCBpeer[d.data.from].datachannel;datachannel.onclose=function(e){console.log("DataChannelObject onclose from: "+d.data.from),console.log(e)},datachannel.onerror=function(e){console.log("DataChannelObject onerror from: "+d.data.from),console.log(e)},datachannel.onmessage=function(e){console.log("DataChannelObject onmessage from: "+d.data.from),console.log(e),handleDataChannelReceiveMessages(d,e)},datachannel.onopen=function(e){console.log("DataChannelObject onopen from: "+d.data.from),console.log(e),sendFilePresentation(d.data.from)},peer.addStream(VCB.localStream),peer.onicecandidate=function(e){console.log("Info: onicecandidate send from this PC to "+d.data.from),console.log(e);var tmpCandidate={candidate:e.candidate.candidate,sdpMLineIndex:e.candidate.sdpMLineIndex,sdpMid:e.candidate.sdpMid},res={status:!0,data:{from:this.id,to:d.data.from,candidate:tmpCandidate}};socket.emit("on-ice-candidate",res)},peer.onsignalingstatechange=function(e){console.log("Info: onsignalingstatechange from "+d.data.from),console.log(e)},peer.oniceconnectionstatechange=function(e){console.log("Info: oniceconnectionstatechange from "+d.data.from),console.log(e)},peer.onaddstream=function(e){console.log("Info: onaddstream from "+d.data.from),console.log(e),VCBpeer[d.data.from].remoteStream=e;var remoteStream=window.URL.createObjectURL(e.stream);initNewStream(d.data.from,remoteStream,!1)},peer.onremovestream=function(e){console.log("Info: onremovestream from "+d.data.from),console.log(e)},peer.createOffer(function(e){console.log("Info: createOffer from this PC to "+d.data.from),console.log(e),VCBpeer[d.data.from].session.local=e;var res={status:!0,data:{to:d.data.from,session:{sdp:e.sdp,type:e.type}}};socket.emit("get-offer",res)})},handleGetOffer=function(d){if(d.data.client.sessionID!=socket.socket.sessionid){var joinMessage=$("#join-room-message-template").html().replace("{{name}}",d.data.client.name);$("#chat-list").prepend(joinMessage)}console.log("Info: handleGetOffer with input param @d:"),console.log(d),VCBpeer[d.data.from]=new VCBpeerConstructor,VCBpeer[d.data.from].peer=new webkitRTCPeerConnection(null,{optional:[{RtpDataChannels:!0}]});var peer=VCBpeer[d.data.from].peer;peer.addStream(VCB.localStream),peer.onicecandidate=function(e){console.log("Info: onicecandidate send from this PC to "+d.data.from),console.log(e);var tmpCandidate={candidate:e.candidate.candidate,sdpMLineIndex:e.candidate.sdpMLineIndex,sdpMid:e.candidate.sdpMid},res={status:!0,data:{to:d.data.from,candidate:tmpCandidate}};socket.emit("on-ice-candidate",res)},peer.onsignalingstatechange=function(e){console.log("Info: onsignalingstatechange from "+d.data.from),console.log(e)},peer.oniceconnectionstatechange=function(e){console.log("Info: oniceconnectionstatechange from "+d.data.from),console.log(e)},peer.onaddstream=function(e){console.log("Info: onaddstream from ",d),console.log(e),VCBpeer[d.data.from].remoteStream=e;var newStreamURL=window.URL.createObjectURL(e.stream);d.data.from==roomInfo.creator.sessionID?initMainStream(newStreamURL):initNewStream(d.data.from,newStreamURL,!1)},peer.onremovestream=function(e){console.log("Info: onremovestream from "+d.data.from),console.log(e)},peer.ondatachannel=function(e){console.log("Info: ondatachannel from "+d.data.from),console.log(e),VCBpeer[d.data.from].datachannel=e.channel;var datachannel=e.channel;datachannel.onclose=function(e){console.log("getOffer peer DataChannelObject onclose from: ",d.data.from),console.log(e)},datachannel.onerror=function(e){console.log("getOffer peer DataChannelObject onerror from: ",d.data.from),console.log(e)},datachannel.onmessage=function(e){console.log("getOffer peer DataChannelObject onmessage from: ",d.data.from),console.log(e),handleDataChannelReceiveMessages(d,e)},datachannel.onopen=function(e){console.log("getOffer peer DataChannelObject onopen",d),console.log(e)}};var SessionDescription=new RTCSessionDescription({sdp:d.data.session.sdp,type:d.data.session.type});VCBpeer[d.data.from].session.remote=SessionDescription,peer.setRemoteDescription(SessionDescription),peer.createAnswer(function(e){console.log("Info: createAnswer from this PC to "+d.data.from),console.log(e),VCBpeer[d.data.from].session.local=e;var res={status:!0,data:{to:d.data.from,session:{sdp:e.sdp,type:e.type}}};socket.emit("get-answer",res)})},handleGetAnswer=function(d){console.log("Info: handleGetAnswer with input param @d:");var peer=VCBpeer[d.data.from].peer,SessionDescription=new RTCSessionDescription({sdp:d.data.session.sdp,type:d.data.session.type});VCBpeer[d.data.from].session.remote=SessionDescription,peer.setLocalDescription(VCBpeer[d.data.from].session.local),peer.setRemoteDescription(VCBpeer[d.data.from].session.remote);var res={status:!0,data:{to:d.data.from}};socket.emit("get-last-description",res)},handleGetLastDescription=function(d){console.log("Info: handleGetLastDescription with input param @d:"),console.log(d);var peer=VCBpeer[d.data.from].peer;peer.setLocalDescription(VCBpeer[d.data.from].session.local)},handleOnIceCandidate=function(d){console.log("Info: handleOnIceCandidate with input param @d:"),console.log(d);var peer=VCBpeer[d.data.from].peer,IceCandidate=new RTCIceCandidate(d.data.candidate);peer.addIceCandidate(IceCandidate)},handleLeaveRoom=function(d){if(d.creator==d.client||d.client==socket.socket.session)VCB.localStream.stop(),$("#dashboard-content").html($("#dashboard-home-template").html()),VCBpeer=new Object,Vpeer=new Object,pptFiles={data:new Array,size:0};else{$("#"+d.client).remove(),VCBpeer[d.client]=void 0;var leaveMessage=$("#leave-room-message-template").html().replace("{{name}}",d.name);$("#chat-list").prepend(leaveMessage)}},handleInitPresentation=function(d){var tmp={data:d.data.file};handleDataChannelReceiveMessages(d,tmp),$("#slide-current").attr("src",pptFiles.data[0]),$("#slide-current-num").html(currentSlide+1+"/"+pptFiles.data.length),$("#main-content").on("click","#slide-current",function(){$("#slide-current")[0].webkitRequestFullScreen()})},handleControlPresentation=function(d){currentSlide=d.data.page,$("#slide-current-num").html(currentSlide+1+"/"+pptFiles.data.length),$("#slide-current").attr("src",pptFiles.data[currentSlide])},recordStream=function(){if(VCB.recorder)return!0;var streamToRecord,creatorSessionID=VCB.roomInfo.creator.sessionID,currentSessionID=socket.socket.sessionid;streamToRecord=creatorSessionID==currentSessionID?VCB.localstream:VCBpeer[creatorSessionID].remoteStream.stream,console.log(streamToRecord),VCB.recorder=new recordWebM(streamToRecord),VCB.recorder.init()},stopRecordStream=function(){return VCB.recorder?(VCB.recorder.stop(),void(VCB.recorder=void 0)):!1};$("#main-content").on("click","#record-stream-button",function(e){e.preventDefault();var control=$(this).attr("data-control");"record"==control?($(this).removeClass("record-button").addClass("stop-button").html("Stop").attr("data-control","stop"),recordStream()):($(this).removeClass("stop-button").addClass("record-button").html("Record").attr("data-control","record"),stopRecordStream())}),$("#main-content").on("submit","#chat",function(e){e.preventDefault();var message=$("#chatbox")[0];if(message.value.length>0){var chatTemplate=$("#chat-template").html();chatTemplate=chatTemplate.replace("{{name}}",VCB.client.name),chatTemplate=chatTemplate.replace("{{message}}",message.value),$("#chat-list").prepend(chatTemplate);for(var client in VCBpeer)chat.send(VCBpeer[client].datachannel,message.value)}message.value=null}),socket.on("ask-offer",handleAskOffer),socket.on("get-offer",handleGetOffer),socket.on("get-answer",handleGetAnswer),socket.on("get-last-description",handleGetLastDescription),socket.on("on-ice-candidate",handleOnIceCandidate),socket.on("init-presentation",handleInitPresentation),socket.on("control-presentation",handleControlPresentation),socket.on("leave-room",handleLeaveRoom)},leaveRoom=function(){socket.emit("leave-room"),VCB.localStream.stop()},initStream=function(d){1==d.status?(VCB.roomid=d.roomInfo.id,VCB.roomInfo=d.roomInfo,startStream(d.roomInfo)):(VCB.localStream.stop(),VCB.localStream=void 0,$("#message").addClass("error").html("ERROR: "+d.error).fadeIn(750,function(){$(this).delay(1e3).fadeOut(750)}))},showStatusSTUN=function(d){console.log(d)};